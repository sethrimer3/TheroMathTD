...
/** Module namespace shape produced by a JSON dynamic import. */
type JsonModuleNamespace = { default?: unknown } | null | undefined;

/** Narrow arbitrary global payloads to inspectable objects. */
 ...
 */
export function resolveFallbackUrl(relativePath: string): string | null {
...
export async function fetchJsonWithFallback(
  urlPrimary: string | null | undefined,
  relativePath: string,
): Promise<unknown> {
...
export function getEmbeddedGameplayConfig(
  globalKey: string = EMBEDDED_CONFIG_GLOBAL_KEY,
  ...
  const embedded: unknown = Reflect.get(root, globalKey);
  return embedded && isObjectLike(embedded) ? embedded : null;
}
...
let cachedJsonModuleImporter: ((specifier: string) => Promise<JsonModuleNamespace>) | null | undefined = undefined;

function getJsonModuleImporter(): ((specifier: string) => Promise<JsonModuleNamespace>) | null {
  ...
  try {
    ...
    cachedJsonModuleImporter = (specifier: string) => Reflect.apply(importer, undefined, [specifier]);
  } catch (_e) {
    cachedJsonModuleImporter = null;
  }
 ...
 */
export async function importJsonModule(moduleUrl: string | null | undefined): Promise<unknown> {
  if (!moduleUrl) {
    return null;
  }
...
export async function loadGameplayConfigViaModule(moduleUrl: string | null | undefined): Promise<unknown> {
