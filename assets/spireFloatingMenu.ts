import type { TabId } from './uiTabManager.js';

/** Identifiers for the floating "Spire" trays that live beneath each spire tab. */
export type SpireTabId = 'powder' | 'fluid' | 'lamed' | 'tsadi' | 'shin' | 'kuf';

export interface SpireMenuControllerOptions {
  formatGameNumber?: (value: number) => string;
  formatWholeNumber?: (value: number) => string;
  getCurrentIdleMoteBank?: () => number;
  getCurrentFluidDropBank?: () => number;
  getLamedSparkBank?: () => number;
  getTsadiParticleBank?: () => number;
  getShinGlyphs?: () => number;
  getKufGlyphs?: () => number;
  isFluidUnlocked?: () => boolean;
  isLamedUnlocked?: () => boolean;
  isTsadiUnlocked?: () => boolean;
  isShinUnlocked?: () => boolean;
  isKufUnlocked?: () => boolean;
  setActiveTab?: (target: TabId) => void;
  playMenuSelectSfx?: () => void;
}

export interface SpireMenuController {
  initialize: () => void;
  updateCounts: () => void;
}
 ...
 */
export function createSpireFloatingMenuController(
  options: SpireMenuControllerOptions = {},
): SpireMenuController {
  /**
   ...
   */
  const {
    formatGameNumber = (value: number) => String(value ?? 0),
    formatWholeNumber = (value: number) => String(value ?? 0),
  ...
  function initialize(): void {
    ...
    const menuItems = doc.querySelectorAll<HTMLElement>('.spire-menu-item');
    menuItems.forEach((item) => {
      item.addEventListener('click', (event) => {
        event.preventDefault();
        const targetTab = item.dataset.tab as TabId | undefined;
        if (!targetTab) {
          return;
        }
    ...
    const spireIds: SpireTabId[] = ['powder', 'fluid', 'lamed', 'tsadi', 'shin', 'kuf'];
     ...
     */
    function collapseMenuForToggle(
    ...
    spireIds.forEach((spireId) => {
      const toggleButton = doc.getElementById(`spire-menu-toggle-${spireId}`) as HTMLElement | null;
      const menu = doc.getElementById(`spire-floating-menu-${spireId}`) as HTMLElement | null;
      ...
      if (toggleButton && menu) {
        ...
        toggleButton.addEventListener('click', () => {
          ...
          doc.querySelectorAll<HTMLElement>('.spire-floating-menu').forEach((otherMenu) => {
            if (otherMenu === menu) {
              return;
            }
            otherMenu.classList.remove(visibleClass);
            const otherId = otherMenu.id.replace('spire-floating-menu-', '');
            const otherToggle = doc.getElementById(`spire-menu-toggle-${otherId}`) as HTMLElement | null;
            if (otherToggle) {
  ...
  /**
   * Refresh a single locked/unlocked spire menu item collection, toggling the
   * hidden/aria-hidden state and swapping the icon glyph for a lock placeholder.
   * Behavior is identical to (and replaces five previously duplicated blocks of)
   * the original per-spire aria-hidden/icon-swap logic.
   */
  function refreshLockableSpireItems(
    doc: Document,
    selector: string,
    isUnlocked: () => boolean,
  ): void {
    doc.querySelectorAll<HTMLElement>(selector).forEach((item) => {
      const iconElement = item.querySelector<HTMLElement>('.spire-menu-icon');
      if (isUnlocked()) {
  ...
  function updateCounts(): void {
    ...
    refreshLockableSpireItems(doc, '.spire-menu-item--bet', isFluidUnlocked);
    refreshLockableSpireItems(doc, '.spire-menu-item--lamed', isLamedUnlocked);
    refreshLockableSpireItems(doc, '.spire-menu-item--tsadi', isTsadiUnlocked);
    refreshLockableSpireItems(doc, '.spire-menu-item--shin', isShinUnlocked);
    refreshLockableSpireItems(doc, '.spire-menu-item--kuf', isKufUnlocked);
  }
