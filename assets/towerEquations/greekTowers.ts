...
function multiplyUnknowns(first: unknown, second: unknown): number {
  return Number(first) * Number(second);
}

/** Preserve formatDecimal's native non-finite fallback for a JavaScript-originated value. */
function formatDecimalWithUnknown(value: unknown, digits: number): string {
  ...
  variables: [
    {
      ...
      computeValue({ blueprint, towerId }) {
        const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
        const gammaValue = Math.max(0, requireHelper('calculateTowerEquationResult')('gamma'));
        const aleph1 = mathMaxWithUnknown(
          1,
          requireHelper('computeTowerVariableValue')(towerId, 'aleph1', effectiveBlueprint),
        );
        const critical = mathMaxWithUnknown(
          0,
          requireHelper('computeTowerVariableValue')(towerId, 'crt', effectiveBlueprint),
        );
    ...
    },
    {
      ...
      getSubEquations({ blueprint, towerId }) {
        const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
        const aleph2 = mathMaxWithUnknown(
          1,
          requireHelper('computeTowerVariableValue')(towerId, 'aleph2', effectiveBlueprint),
        );
        const aleph3 = mathMaxWithUnknown(
          1,
          requireHelper('computeTowerVariableValue')(towerId, 'aleph3', effectiveBlueprint),
        );
        const aleph4 = mathMaxWithUnknown(
          1,
          requireHelper('computeTowerVariableValue')(towerId, 'aleph4', effectiveBlueprint),
        );
        const aleph5 = mathMaxWithUnknown(
          1,
          requireHelper('computeTowerVariableValue')(towerId, 'aleph5', effectiveBlueprint),
        );
    ...
    },
    {
      ...
      computeValue({ blueprint, towerId }) {
        const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
        const aleph6 = mathMaxWithUnknown(
          1,
          requireHelper('computeTowerVariableValue')(towerId, 'aleph6', effectiveBlueprint),
        );
