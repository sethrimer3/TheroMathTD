 ...
 */
import { formatWholeNumber, formatDecimal, formatGameNumber, formatPercentage, } from '../../scripts/core/formatting.js';
...
/** Preserve numeric multiplication's native coercion for JavaScript-originated values. */
function multiplyUnknowns(first, second) {
    return Number(first) * Number(second);
}
/** Preserve formatDecimal's native non-finite fallback for a JavaScript-originated value. */
function formatDecimalWithUnknown(value, digits) {
    return Reflect.apply(formatDecimal, undefined, [value, digits]);
}
...
export const delta = {
    ...
    variables: [
        ...
        },
        {
            ...
            equationSymbol: 'ℵ₁',
            name: 'Aleph₁ Phalanx',
            description: 'Allocates ℵ₁ glyphs to Δ soldiers, amplifying vitality, muster, and training cadence.',
            baseValue: 1,
            step: 1,
            upgradable: true,
            ...
            getSubEquations({ value }) {
                ...
                const gammaDefinition = requireHelper('getTowerDefinition')('gamma');
                const gammaEquation = requireHelper('calculateTowerEquationResult')('gamma');
                const fallbackGamma = Number.isFinite(gammaDefinition?.damage)
                    ? mathMaxWithUnknown(1, gammaDefinition?.damage)
                    : 1;
        ...
        },
        {
            ...
            computeValue({ blueprint, towerId }) {
                ...
                const alephValue = requireHelper('computeTowerVariableValue')(towerId, 'aleph1', effectiveBlueprint);
                const alephRank = isFiniteNumber(alephValue) ? Math.max(1, Math.round(alephValue)) : 1;
                const gammaDefinition = requireHelper('getTowerDefinition')('gamma');
                const gammaEquation = requireHelper('calculateTowerEquationResult')('gamma');
                const fallbackGamma = Number.isFinite(gammaDefinition?.damage)
                    ? mathMaxWithUnknown(1, gammaDefinition?.damage)
        ...
        },
        {
            ...
            equationSymbol: 'ℵ₂',
            name: 'Aleph₂ Shift',
            description: 'Delta shift force that knocks enemies backward along the track when a Δ soldier rams and dies.',
            baseValue: 0,
            step: 1,
            upgradable: true,
            includeInMasterEquation: false,
    ...
    computeResult(values) {
        const gammaValue = Math.max(0, isFiniteNumber(values.gamma) ? values.gamma : 0);
    ...
    variables: [
        {
            ...
            getSubEquations({ blueprint, towerId, level: _level, value }) {
                const _effective = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
...
export const zeta = {
    ...
    variables: [
        ...
        },
        {
            ...
            computeValue({ blueprint, towerId }) {
                const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
                const aleph5 = requireHelper('computeTowerVariableValue')(towerId, 'aleph5', effectiveBlueprint);
                const aleph6 = requireHelper('computeTowerVariableValue')(towerId, 'aleph6', effectiveBlueprint);
                const product = Math.max(1, multiplyUnknowns(aleph5, aleph6));
            ...
            getSubEquations({ blueprint, towerId }) {
                ...
                return [
                    ...
                    {
                        values: String.raw `\( ${formatDecimal(product, 2)} = ${formatDecimalWithUnknown(aleph5, 2)} \times ${formatDecimalWithUnknown(aleph6, 2)} \)`,
                        variant: 'values',
                    },
                ];
            },
        },
        {
            ...
            computeValue({ blueprint, towerId }) {
                const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
                const gammaValue = Math.max(0, requireHelper('calculateTowerEquationResult')('gamma'));
                const aleph1 = mathMaxWithUnknown(1, requireHelper('computeTowerVariableValue')(towerId, 'aleph1', effectiveBlueprint));
                const critical = mathMaxWithUnknown(1, requireHelper('computeTowerVariableValue')(towerId, 'crt', effectiveBlueprint));
        ...
        },
        {
            ...
            computeValue({ blueprint, towerId }) {
                const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
                const aleph2 = mathMaxWithUnknown(0, requireHelper('computeTowerVariableValue')(towerId, 'aleph2', effectiveBlueprint));
        ...
        },
        {
            ...
            computeValue({ blueprint, towerId }) {
                const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
                const aleph3 = mathMaxWithUnknown(0, requireHelper('computeTowerVariableValue')(towerId, 'aleph3', effectiveBlueprint));
        ...
        },
        {
            ...
            computeValue({ blueprint, towerId }) {
                const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
                const aleph4 = mathMaxWithUnknown(0, requireHelper('computeTowerVariableValue')(towerId, 'aleph4', effectiveBlueprint));
    ...
    computeResult(values) {
        ...
        const critical = isFiniteNumber(values.crt) ? values.crt : 1;
        const speed = isFiniteNumber(values.spd) ? values.spd : 0;
        const range = isFiniteNumber(values.rng) ? values.rng : 0;
        const total = isFiniteNumber(values.tot) ? values.tot : 0;
    ...
    variables: [
        {
            ...
            computeValue({ blueprint, towerId }) {
                ...
                const gammaValue = Math.max(0, requireHelper('calculateTowerEquationResult')('gamma'));
                const aleph1 = mathMaxWithUnknown(1, requireHelper('computeTowerVariableValue')(towerId, 'aleph1', effectiveBlueprint));
                const critical = mathMaxWithUnknown(0, requireHelper('computeTowerVariableValue')(towerId, 'crt', effectiveBlueprint));
            ...
            getSubEquations({ blueprint, towerId }) {
                ...
                return [
                    ...
                    {
                        values: String.raw `\( ${formatGameNumber(attack)} = (${formatDecimal(gammaValue, 2)} \cdot ${formatWholeNumber(aleph1)})^{${formatDecimal(critical, 2)}} \)`,
                        variant: 'values',
                    },
                ];
            },
        ...
        },
        {
            ...
            computeValue({ blueprint, towerId }) {
                const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
                const bet1 = mathMaxWithUnknown(0, requireHelper('computeTowerVariableValue')(towerId, 'bet1', effectiveBlueprint));
        ...
        },
        {
            ...
            getSubEquations({ blueprint, towerId }) {
                const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
                const aleph2 = mathMaxWithUnknown(1, requireHelper('computeTowerVariableValue')(towerId, 'aleph2', effectiveBlueprint));
                const aleph3 = mathMaxWithUnknown(1, requireHelper('computeTowerVariableValue')(towerId, 'aleph3', effectiveBlueprint));
                const aleph4 = mathMaxWithUnknown(1, requireHelper('computeTowerVariableValue')(towerId, 'aleph4', effectiveBlueprint));
                const aleph5 = mathMaxWithUnknown(1, requireHelper('computeTowerVariableValue')(towerId, 'aleph5', effectiveBlueprint));
                ...
                const values = [
                    ...
                    {
                        values: String.raw `\( ${formatDecimal(1 / denominator, 3)} = \frac{1}{${formatWholeNumber(aleph2)} + ${formatWholeNumber(aleph3)} + ${formatWholeNumber(aleph4)} + ${formatWholeNumber(aleph5)}} \)`,
                        variant: 'values',
                    },
                    {
                    ...
                    {
                        values: String.raw `\( ${formatDecimal((1 + aleph2) / denominator, 3)} = \frac{1 + ${formatWholeNumber(aleph2)}}{${formatWholeNumber(aleph2)} + ${formatWholeNumber(aleph3)} + ${formatWholeNumber(aleph4)} + ${formatWholeNumber(aleph5)}} \)`,
                        variant: 'values',
                    },
                    {
                    ...
                    {
                        values: String.raw `\( ${formatDecimal((1 + 2 * aleph3) / denominator, 3)} = \frac{1 + 2 \cdot ${formatWholeNumber(aleph3)}}{${formatWholeNumber(aleph2)} + ${formatWholeNumber(aleph3)} + ${formatWholeNumber(aleph4)} + ${formatWholeNumber(aleph5)}} \)`,
                        variant: 'values',
                    },
                    {
                    ...
                    {
                        values: String.raw `\( ${formatDecimal((2 + 3 * aleph4) / denominator, 3)} = \frac{2 + 3 \cdot ${formatWholeNumber(aleph4)}}{${formatWholeNumber(aleph2)} + ${formatWholeNumber(aleph3)} + ${formatWholeNumber(aleph4)} + ${formatWholeNumber(aleph5)}} \)`,
                        variant: 'values',
                    },
                    {
                    ...
                    {
                        values: String.raw `\( ${formatDecimal((1 + 2 ** aleph5) / denominator, 3)} = \frac{1 + 2^{${formatWholeNumber(aleph5)}}}{${formatWholeNumber(aleph2)} + ${formatWholeNumber(aleph3)} + ${formatWholeNumber(aleph4)} + ${formatWholeNumber(aleph5)}} \)`,
                        variant: 'values',
                    },
                ];
                return values;
        ...
        },
        {
            ...
            computeValue({ blueprint, towerId }) {
                const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
                const aleph6 = mathMaxWithUnknown(1, requireHelper('computeTowerVariableValue')(towerId, 'aleph6', effectiveBlueprint));
...
export const theta = {
    ...
    variables: [
        ...
        },
        {
            ...
            computeValue({ blueprint, towerId }) {
                const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
                const aleph1 = mathMaxWithUnknown(0, requireHelper('computeTowerVariableValue')(towerId, 'aleph1', effectiveBlueprint));
            ...
            getSubEquations({ blueprint, towerId }) {
                ...
                return [
                    ...
                    {
                        values: String.raw `\( ${formatDecimal(clamped, 2)}\% = 95 \left( 1 - e^{-0.1 \cdot ${formatDecimal(aleph1, 2)}} \left( 1 + 0.1 \sin(${formatDecimal(aleph1, 2)}) \right) \right) + 5 \)`,
                        variant: 'values',
                    },
                ];
            },
        ...
        },
        {
            ...
            getSubEquations({ blueprint, towerId }) {
                const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
                const aleph2 = mathMaxWithUnknown(1, requireHelper('computeTowerVariableValue')(towerId, 'aleph2', effectiveBlueprint));
                const _aleph3 = mathMaxWithUnknown(0, requireHelper('computeTowerVariableValue')(towerId, 'aleph3', effectiveBlueprint));
    ...
    computeResult(values) {
        const range = isFiniteNumber(values.rng) ? values.rng : 0;
        const slow = isFiniteNumber(values.slw) ? values.slw : 0;
        return range * slow;
    },
...
export const iota = {
    ...
    variables: [
        ...
        },
        {
            ...
            computeValue({ blueprint, towerId }) {
                const effectiveBlueprint = blueprint || requireHelper('getTowerEquationBlueprint')(towerId);
                const alphaLinks = Math.max(0, requireHelper('getDynamicConnectionCount')('alpha'));
                const betaLinks = Math.max(0, requireHelper('getDynamicConnectionCount')('beta'));
                const gammaLinks = Math.max(0, requireHelper('getDynamicConnectionCount')('gamma'));
                const aleph0 = mathMaxWithUnknown(0, requireHelper('computeTowerVariableValue')('iota', 'aleph0', effectiveBlueprint));
                const aleph1 = mathMaxWithUnknown(0, requireHelper('computeTowerVariableValue')('iota', 'aleph1', effectiveBlueprint));
                const aleph2 = mathMaxWithUnknown(0, requireHelper('computeTowerVariableValue')('iota', 'aleph2', effectiveBlueprint));
                const aleph3 = mathMaxWithUnknown(0, requireHelper('computeTowerVariableValue')('iota', 'aleph3', effectiveBlueprint));
    ...
    computeResult(values) {
        ...
        const speed = isFiniteNumber(values.spd) ? values.spd : 0;
        const rangeMeters = isFiniteNumber(values.rangeMeters) ? values.rangeMeters : 0;
