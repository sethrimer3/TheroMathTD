...
function clampChiValue(value: number, min: number, max: number): number {
...
function resolvePhiPower(): number {
  const helpers = ctx();
...
function resolveChiCorePower(): number {
...
export const chi = {
  ...
  variables: [
    ...
    },
    {
      ...
      upgradable: false,
      includeInMasterEquation: true,
      format: (value) => formatPercentage(Math.max(0, value || 0)),
      computeValue() {
      ...
      getSubEquations({ value }) {
        ...
        return [
          ...
          {
            values: String.raw`\( ${formatPercentage(health)} = \operatorname{clamp}(0.28 + ${formatDecimal(normalized, 2)} \times 0.05,\,0.25,\,0.85) \)`,
            variant: 'values',
          },
        ];
      },
  ...
  formatBaseEquationValues({ values, result, formatComponent }) {
    ...
    const speedBonus = Math.max(0, Number(values.speedBonus) || 0);
    const thralls = Math.max(1, Number(values.maxThralls) || 1);
    return `${formatComponent(result)} = ${formatComponent(core)} × ${formatPercentage(health)} × ${formatComponent(1 + speedBonus)} × ${formatComponent(thralls)}`;
  },
