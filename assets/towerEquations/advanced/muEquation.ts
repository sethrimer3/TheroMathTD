...
/** Invoke the required equation-result helper while preserving the initialized-context contract. */
function calculateEquationResult(towerId: string): number {
  const calculateResult = ctx().calculateTowerEquationResult;
  if (typeof calculateResult !== 'function') {
    throw new TypeError('ctx(...).calculateTowerEquationResult is not a function');
  }
  return calculateResult(towerId);
}
...
export const mu = {
  ...
  variables: [
    {
      ...
      getSubEquations({ level, value }) {
        ...
        const lambdaValue = isFiniteNumber(lambdaRaw) ? Math.max(0, lambdaRaw) : 0;
  ...
  computeResult(values) {
    ...
    const lambdaValue = calculateEquationResult('lambda');
    const lambda = isFiniteNumber(lambdaValue) ? Math.max(0, lambdaValue) : 0;
    const tier = Math.max(1, isFiniteNumber(values.aleph1) ? Math.round(values.aleph1) : 1);
