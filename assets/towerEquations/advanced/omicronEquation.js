...
export const omicron = {
    ...
    variables: [
        {
            ...
            getSubEquations({ blueprint: _blueprint, towerId: _towerId, value }) {
                ...
                const xiValue = Math.max(0, Number.isFinite(helpers.calculateTowerEquationResult?.('xi'))
                    ? Number(helpers.calculateTowerEquationResult?.('xi'))
                    : 0);
                ...
                const deltaAleph1 = Math.max(1, Number.isFinite(helpers.computeTowerVariableValue?.('delta', 'aleph1', deltaBlueprint))
                    ? Number(helpers.computeTowerVariableValue?.('delta', 'aleph1', deltaBlueprint))
                    : 1);
        ...
        },
        {
            ...
            getSubEquations({ blueprint: _blueprint, towerId: _towerId, value }) {
                ...
                const deltaValue = Math.max(0, Number.isFinite(helpers.calculateTowerEquationResult?.('delta'))
                    ? Number(helpers.calculateTowerEquationResult?.('delta'))
                    : 0);
                const attackRaw = deltaValue * xiValue;
                const attack = Number.isFinite(attackRaw) ? attackRaw : 0;
                return [
    ...
    computeResult(values) {
        const helpers = ctx();
        const deltaValue = Math.max(0, isFiniteNumber(values.delta)
            ? values.delta
            : Number.isFinite(helpers.calculateTowerEquationResult?.('delta'))
                ? Number(helpers.calculateTowerEquationResult?.('delta'))
                : 0);
        const xiValue = Math.max(0, isFiniteNumber(values.xi)
            ? values.xi
