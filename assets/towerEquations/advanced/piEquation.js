...
export const pi = {
    ...
    variables: [
        {
            ...
            getSubEquations({ blueprint, towerId, value }) {
                ...
                const bet1 = Math.max(0, Math.min(99, isFiniteNumber(bet1Raw) ? bet1Raw : 0));
        ...
        },
        {
            ...
            getSubEquations({ blueprint, towerId, level, value }) {
                ...
                const omicronValue = Math.max(1, isFiniteNumber(omicronRaw) ? omicronRaw : 1);
    ...
    computeResult(values) {
        const helpers = ctx();
        const omicronRaw = isFiniteNumber(values.omicron)
            ? values.omicron
            : helpers.calculateTowerEquationResult?.('omicron') || 1;
        const omicronValue = Math.max(1, Number.isFinite(omicronRaw) ? omicronRaw : 1);
        const bet1 = Math.max(0, Math.min(99, isFiniteNumber(values.bet1) ? values.bet1 : 0));
        const divisor = Math.max(1, 100 - bet1);
