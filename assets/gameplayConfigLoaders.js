...
export function getEmbeddedGameplayConfig(globalKey = EMBEDDED_CONFIG_GLOBAL_KEY) {
    ...
    const embedded = Reflect.get(root, globalKey);
...
function getJsonModuleImporter() {
    ...
    try {
        ...
        const importer = Function('specifier', "return import(specifier, { assert: { type: 'json' } });");
        cachedJsonModuleImporter = (specifier) => Reflect.apply(importer, undefined, [specifier]);
    }
    catch (_e) {
        cachedJsonModuleImporter = null;
    }
