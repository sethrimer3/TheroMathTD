...
Using the established count method, the current tree contains **48 authored `.ts` modules** and **259 authored `.js` modules without a same-path `.ts` sibling** (307 total). Of those 259 JavaScript modules, **19 are explicitly disabled legacy Achievements Terrarium/Bet Terrarium files** documented at `assets/legacy/achievementsTerrarium/README.md`; they are preserved for possible restoration and are not part of the active migration backlog. The active remaining JavaScript backlog is therefore **240 modules**. Phase 7 added `assets/alephUpgradeState.ts` and replaced Phase 6's named external Aleph snapshot/playfield boundary with the owner module's real types without changing the total authored-module count. The retired `spireFloatingMenu` and `spireTabVisibility` TypeScript modules and the orphaned retired idle/developer controllers were removed because the one-Well navigation no longer imports or needs those multi-Spire managers. The Collective Unconscious/Cognitive Realm is unchanged by this feature task.
...
- **PARTIALLY RESOLVED by Phases 6–7 (2026-07-15):** Phase 6 migrated the post-retirement persistence owner and narrowed both Spire-resource hooks plus both tower/Aleph wrapper hooks. Phase 7 then migrated `assets/alephUpgradeState.ts` and replaced the Aleph snapshot/playfield boundary with its real owner types. Only the base tower-upgrade snapshot remains a named external boundary, owned by `assets/towerBlueprintPresenter.js`.
...
**Recommended Phase 8: `assets/towerBlueprintPresenter.js` (tower equation presenter and base tower-upgrade persistence owner).**

Phase 7 closes the Aleph half of the tower/Aleph save wrapper, leaving `ExternalTowerUpgradeSnapshot` as the sole external schema in `spireResourcePersistence.ts`. The live owner is the 321-line `assets/towerBlueprintPresenter.js`, directly imported only by `assets/towersTab.js`; it owns the upgrade map, snapshot getter/restore, glyph-cost aggregation, fallback-blueprint construction, equation-result cache, and reset API. Migrating this single factory next would close the remaining autosave boundary while keeping `assets/towersTab.js`, `assets/main.js`, and the authored `assets/towerEquations/**` modules unconverted.

**Bounded scope:** Convert `assets/towerBlueprintPresenter.js` to strict TypeScript as one behavior-preserving owner; model the exact tower-upgrade snapshot and minimal injected definition/blueprint/formatter contracts; add deterministic compiled-output tests for snapshot normalization/restoration, cost progression, fallback blueprints, cache invalidation/recursion protection, and targeted/all-state clearing; update `spireResourcePersistence.ts` and autosave wrapper types to reuse the new snapshot contract. Do not convert `assets/towersTab.js`, `assets/main.js`, `assets/towerEquations/**`, tower simulations, or UI overlays.

**Acceptance criteria:** Re-derive every importer and returned-method consumer; preserve factory validation, fallback formatting/equations, lazy state creation, snapshot omission rules, positive-only restore behavior, fractional-level handling, glyph-cost calculation, reference/transform/exponent evaluation, finite fallbacks, warning behavior, caching, and clear semantics; use named minimal boundaries for still-JavaScript equation blueprints; add no `any` or suppressions; typecheck/build/lint/unit/smoke and browser checks pass; update counts/plans/build number exactly once.
