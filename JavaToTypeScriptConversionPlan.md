...
**Completion record:** Phase 24 converted exactly the two authorized grouped definition files, retained all `.js` import/export specifiers and exact registry identity for Alpha, Beta, Gamma, and Infinity, and reused the presenter/context contracts. Because these modules call context helpers without optional chaining, each source uses a small `requireHelper` guard that preserves the original uninitialized-context `TypeError`; Beta's unknown-typed glyph read reuses the established native-`Math.max`-coercion helper, and Infinity's `unspentThero` read preserves the original two-read property-getter count. The only shared owner change adds the already-authored `renderControlsInline` field to `TowerEquationVariable`, proven necessary by the compiler against Beta's `betSlow` variable and already consumed by `assets/towerUpgradeOverlayController.js`. Ten focused tests passed first against the original JavaScript and then against generated TypeScript output. Final command evidence and the unperformed optional browser inspection are recorded in the Phase 24 implementation-log entry.

#### Phase 25 authorization card

**Intent:** migrate the Greek grouped equation monolith after Phase 24 completed the basic and Infinity grouped definitions. This 1,684-line file owns the Delta, Epsilon, Zeta, Eta, Theta, and Iota definitions and completes the authored equation-definition backlog consumed by the typed registry.

**Exact authored sources:**

- `assets/towerEquations/greekTowers.js` (Delta, Epsilon, Zeta, Eta, Theta, and Iota definitions)

Convert the file to one strict `.ts` source and regenerate only its same-path `.js` sibling. Retain all `.js` import/export specifiers and exact registry identity for every exported definition object.

**Dependency boundary:** downstream equations read Greek results only through `blueprintContext`, so no downstream file changes. Reuse `TowerEquationBlueprint`, presenter-owned value types, and `blueprintContext`. Do not convert the Towers tab, simulations, playfield, or main, and do not split or redesign formulas during migration.

**Required characterization:** characterize each of the six towers independently before conversion — exact metadata and variable order; every cost/base/step formula; helper call counts and fallbacks; Number/Boolean coercion, `NaN`, Infinity, negative and fractional cases; every sub-equation branch and formatter order; result/base-equation behavior; registry identity for all six definitions. Tests must be deterministic and pass against original JavaScript before conversion and generated output afterward. An all-equation browser overlay pass is recommended and must be reported as performed or not performed.

**Completion gate:** use the standard implementation gate and phase record. No formula cleanup, balance change, broad assertion, `any`, suppression, file split, or simulation edit is authorized. Completion must refresh counts, inventory coverage, Build metadata, and exactly one next authorization.

**Completion record:** Phase 25 converted the single authorized 1,684-line monolith to one strict `.ts` source, retained all `.js` import/export specifiers, exact registry identity for Delta, Epsilon, Zeta, Eta, Theta, and Iota, and every formula, string, clamp, and fallback. The module reuses the presenter/context contracts and required no shared owner edit. Its non-optionally-chained helper calls use the same `requireHelper` guard proven in Phase 24; raw `unknown` helper reads preserve native `Math.max` coercion, Zeta's critical multiplier preserves native multiplication coercion via a dedicated helper, and Zeta's raw formatter inputs preserve `formatDecimal`'s native non-finite fallback via `Reflect.apply`. Iota's authored keyless Pulse Attack variable is preserved exactly and characterized. Nine focused per-tower tests passed first against the original JavaScript and then against generated TypeScript output. Final command evidence and the unperformed recommended browser overlay pass are recorded in the Phase 25 implementation-log entry.

#### Phase 26 authorization card

**Intent:** migrate the eleven pure utility, token, unit, geometry, wave-codec, playfield-facade, build-data, and Aleph-registry modules after Phase 25 completed the entire authored equation backlog. These are the lowest-risk, highest-fan-in remaining modules and establish the `Wave`, `EnemyGroup`, unit, token, point, and Aleph registry contracts consumed by later configuration, tower, and playfield phases.

**Exact authored sources:** the eleven modules in the Phase 26 inventory row (`assets/buildInfo.js`, `assets/formatHelpers.js`, `assets/gameUnits.js`, `assets/geometryHelpers.js`, `assets/waveEncoder.js`, `assets/playfield/constants.js`, `assets/playfield/utils/formatting.js`, `assets/playfield/utils/math.js`, `scripts/core/mathTokens.js`, `scripts/core/mathUtils.js`, `scripts/features/towers/alephChain.js`).

Convert those files to strict `.ts` sources and regenerate only their same-path `.js` siblings. Retain all `.js` import/export specifiers.

**Dependency boundary:** these modules are widely imported (31 importers for `gameUnits`, 13 for the playfield formatting facade) but import little themselves. Owner-define the wave, unit, point, token, and Aleph registry contracts here; later phases consume them instead of creating parallel primitives. Do not convert configuration/level/enemy loaders, tower simulations, playfield systems, or main.

**Required characterization:** exact export surfaces; edge/malformed/round-trip cases for the wave codec including its coercive legacy behavior; numeric conversion and geometry invariants; token parsing; Aleph registry identity. Tests must be deterministic and pass against original JavaScript before conversion and generated output afterward. No browser verification is required beyond a wave-editor smoke, reported honestly.

**Completion gate:** use the standard implementation gate and phase record. No behavior change, broad assertion, `any`, or suppression is authorized. Completion must refresh counts, inventory coverage, Build metadata, and exactly one next authorization.

**Completion record:** Phase 26 converted exactly the eleven authorized modules, retained every export surface and `.js` import specifier, and owner-defined the `DecodedWave`/`DecodedWaveEnemyGroup`/`DecodedWaveBoss`/`EncodableWave` codec contracts, `NormalizedPoint`/`Point2D` geometry contracts, `EquationVariableToken`/`EquationTokenSpan` token contracts, and `AlephChainUpgrades`/`AlephChainState` registry contracts. Nine focused tests passed first against the original JavaScript and then against generated TypeScript output, characterizing the wave codec's coercive legacy behavior exactly. Two pre-existing defects were discovered by characterization, recorded under Known Issues, and deliberately not fixed: `escapeRegExp`'s malformed character class effectively escapes nothing on typical symbols, and the encoder writes a duplicate zero delay slot for boss waves without an authored delay, which silently drops the boss on decode. Final command evidence is recorded in the Phase 26 implementation-log entry.

#### Phase 27 authorization card

**Intent:** migrate the configuration, level, and enemy owners after Phase 26 established the shared primitive contracts. These four modules validate JSON/global inputs and owner-define the level, wave-source, enemy, and gameplay-configuration records consumed by combat and rendering phases.

**Exact authored sources:** `assets/gameplayConfigLoaders.js`, `assets/levels.js`, `assets/configuration.js`, and `assets/enemies.js`.

Convert those files to strict `.ts` sources and regenerate only their same-path `.js` siblings. Retain all `.js` import/export specifiers.

**Dependency boundary:** treat JSON, fetch, dynamic-import, and global inputs as `unknown` with runtime validation at entry. Reuse the Phase 4 tower-data types and Phase 26 wave contracts. Consumers in main, playfield, and managers keep receiving the same validated records. Do not convert combat systems, tower simulations, playfield systems, or main.
...
**Completion record:** Phase 28 converted exactly the five authorized owners, retained every export surface, persisted key, hook shape, and clone/guard branch. Storage payloads and legacy snapshots stay `unknown`-guarded: tutorial flags read through typed `autoSave` storage, and the Well basin snapshot routes through the typed `migrateWellOfInspirationSave` with the original field-by-field `hasOwnProperty`/finite/string/boolean restore filter. Owner contracts established: `PowderConfig`/`PowderState`/`PowderStateContext`, `PowderMoteDropPayload`, `PowderEventLogDependencies`, and `PowderBasinSnapshot`/`PowderPersistenceOptions`/`PowderPersistenceController`. The still-JavaScript `powderTower.js` palette merger is invoked through a `Reflect.apply` wrapper so its loose signature stays a boundary rather than an assertion. Five focused tests passed first against the original JavaScript and then against generated TypeScript output. Final command evidence and the unperformed recommended browser save/reload check are recorded in the Phase 28 implementation-log entry.

#### Phase 29 authorization card

**Intent:** migrate the Powder grid, palette, and static data utilities after Phase 28 typed the surrounding state and persistence owners. These three modules define the cell, palette, wall, and mote contracts the Phase 30 `PowderSimulation` conversion will consume, without touching the monolith itself.

**Exact authored sources:** `scripts/features/towers/powderGridUtils.js`, `scripts/features/towers/powderPaletteUtils.js`, and `scripts/features/towers/powderTowerData.js`.

Convert those files to strict `.ts` sources and regenerate only their same-path `.js` siblings. Retain all `.js` import/export specifiers.

**Dependency boundary:** owner-define cell/material/wall/mote/palette contracts here; `PowderSimulation` and later palette/render consumers import them. Do not convert `powderTower.js`, Powder UI controllers, playfield systems, or main.
...
- (Phase 26) `scripts/core/mathTokens.ts#escapeRegExp` inherits a malformed character class (`/[.*+?^${}()|[\\]\\]/g`): the class closes at the first `]`, so the pattern only matches a special character immediately followed by a backslash and `]`. Typical symbols therefore pass through unescaped; a symbol containing an unescaped regex metacharacter would still corrupt the tokenizer's dynamic pattern exactly as before. Preserved unchanged and characterized by test.
- (Phase 26) `assets/waveEncoder.ts#encodeWave` writes the zero delay slot twice for a boss wave without an authored delay (both the boss else-branch and the boss-HP branch append `/0`), shifting the boss HP into the fifth slot that `parseWaveSegment`'s four-part destructuring never reads — so such a boss silently disappears on decode. Preserved unchanged and characterized by test; recorded for a future authorized codec fix.
- (Phase 26) `assets/waveEncoder.ts`'s group regexes accept only letters A-O even though `ENEMY_TYPES` defines letters up to T, so waves authored with P-T entries cannot round-trip through the compact format. Preserved unchanged and characterized by test.
...
- **Baseline:** clean `claude/js-to-ts-game-conversion-ijybn5` at Phase 27 closeout commit `ea8a355` (Build 760). Roadmap check (89/89/145/45), typecheck, build, lint, 237/237 unit tests plus retired-Spire checks, and smoke test all passed before implementation.
- **Authorized boundary:** only `assets/tutorialState.js`, `assets/powder/powderState.js`, `assets/powderDropQueue.js`, `assets/powderEventLog.js`, and `assets/powderPersistence.js` moved to strict `.ts`; their generated siblings, five focused tests, roadmap/inventory records, generated `dist/`, and build metadata are included. No Powder-simulation, playfield, or main behavior entered scope, and every persisted key name is unchanged.
- **Boundary typing:** tutorial flags read through the typed `autoSave` storage primitives; the Well basin restore routes through the typed `migrateWellOfInspirationSave` and keeps the original per-field `hasOwnProperty` plus finite/string/boolean filter, array/`structuredClone` copying, forced `'sand'` mode, and hook timing. The still-JavaScript `powderTower.js` palette merger is called through a `Reflect.apply` wrapper, and the internal persistence state reference is captured into a guarded const so strict null analysis holds without assertions.
- **Owner contracts established:** `PowderConfig`/`PowderState`/`AlephTierTransitionState`/`PowderStateContext`, `PowderMoteDropPayload`/`FlushPendingMoteDropsOptions`, `PowderEventLogDependencies`/`PowderBonusSummary`, and `PowderBasinSnapshot`/`PowderPersistenceOptions`/`PowderPersistenceController`.
- **Behavior oracle:** five deterministic tests were added, increasing the suite from 237 to 242, all passing first against the original JavaScript and then against generated TypeScript output. They cover exact storage keys, load/unlock/reset write sequences and the completion checker, the full initial Well state including palette-merge wiring and element registration, drop flush normalization (min-1 rounding, color cloning, queue clearing, missing-simulation no-op), every ledger entry format with retention limit and unshift ordering under a fake DOM, and snapshot deep-clone/migration/guarded-restore round trips including legacy `powder` aliases.
- **Output and counts:** Build 761. Counts are 94 TypeScript sources, 94 generated siblings, 140 active authored JavaScript modules, and 45 decision candidates (40.2% of 234 active authored modules converted). The supplemental line snapshot is 14,943 TypeScript lines and 74,329 active JavaScript lines.
- **Automated evidence:** `npm run check:migration-roadmap` passes at 94/94/140/45; `npm run typecheck`, `npm run build`, `npm run lint`, `npm run test:unit` (242/242 plus retired-Spire checks), `npm test`, and `git diff --check` pass. The malformed-storage fixture prints its expected parse diagnostic while the unit command exits 0.
- **Manual evidence:** the recommended browser save/reload check was not performed; snapshot round trips and legacy-alias migration are covered by compiled-output tests, and this phase changes no rendering, input, or Electron path.
- **Residual risk and handoff:** no known behavior gap or temporary compatibility seam remains. Phase 28 is complete; only Phase 29 (Powder grid/palette/data utilities) is authorized next under its three-module card.
...
- **Baseline:** clean `claude/js-to-ts-game-conversion-ijybn5` at Phase 25 closeout commit `963d229` (Build 758). Roadmap check (74/74/160/45), typecheck, build, lint, 221/221 unit tests plus retired-Spire checks, and smoke test all passed before implementation.
- **Authorized boundary:** exactly the eleven inventory-row modules moved to strict `.ts`: `buildInfo`, `formatHelpers`, `gameUnits`, `geometryHelpers`, `waveEncoder`, `playfield/constants`, `playfield/utils/formatting`, `playfield/utils/math`, `mathTokens`, `mathUtils`, and `alephChain`. Their generated siblings, nine focused tests, roadmap/inventory records, generated `dist/`, and build metadata are included. No configuration/level/enemy loader, simulation, playfield system, or main behavior entered scope.
- **Owner contracts established:** `DecodedWave`/`DecodedWaveEnemyGroup`/`DecodedWaveBoss`/`EncodableWave`/`WaveStringValidation` (wave codec), `NormalizedPoint`/`Point2D` (geometry), `EquationVariableToken`/`EquationTokenSpan` (tokens), and `AlephChainUpgrades`/`AlephChainState`/`AlephChainRegistryOptions` (Aleph registry). Encoder inputs stay permissive with index signatures because callers are still JavaScript; decode outputs are fully typed.
- **Behavior oracle:** nine deterministic tests were added, increasing the suite from 221 to 230, all passing first against the original JavaScript and then against generated TypeScript output. They cover unit conversions and guards, coordinate clamps/orientation transforms/segment distance, easing clamps, combat-number trimming via an identity formatting stub, clamp/lerp/seeded random, regex escaping and tokenization, subscripts/durations/rewards/relative time with mocked `Date.now`, wave parsing (multi-group, boss slot reservation, the A-O letter gate) and encoding/validation including the double-delay-slot quirk and lost-boss round trip, and Aleph chain normalization/squared chaining/overflow clamping.
- **Pre-existing defects recorded, not fixed:** `escapeRegExp`'s malformed character class, the boss round-trip loss from the duplicated zero delay slot, and the A-O-only codec letter gate; also characterized: plain `clamp` propagates `NaN` by design (`clampSafe` is the guarded variant). See Known Issues.
- **Output and counts:** the authored `buildInfo.ts` carries this phase's increment to Build 759. Counts are 85 TypeScript sources, 85 generated siblings, 149 active authored JavaScript modules, and 45 decision candidates (36.3% of 234 active authored modules converted). The supplemental line snapshot is 13,079 TypeScript lines and 75,839 active JavaScript lines.
- **Automated evidence:** `npm run check:migration-roadmap` passes at 85/85/149/45; `npm run typecheck`, `npm run build`, `npm run lint`, `npm run test:unit` (230/230 plus retired-Spire checks), `npm test`, and `git diff --check` pass. The malformed-storage fixture prints its expected parse diagnostic while the unit command exits 0.
- **Manual evidence:** the recommended wave-editor browser smoke was not performed; codec behavior is covered by exact-string parse/encode/validate tests, and this phase changes no DOM, input, Canvas, audio, save, or Electron path.
- **Residual risk and handoff:** no known behavior gap or temporary compatibility seam remains. Phase 26 is complete; only Phase 27 (configuration, level, and enemy owners) is authorized next under its four-module card.

### 2026-07-17 — Phase 25 Greek equation monolith conversion executed

- **Baseline:** clean `claude/js-to-ts-game-conversion-ijybn5` at Phase 24 closeout commit `738e2c2` (Build 757). Roadmap check (73/73/161/45), typecheck, build, lint, 212/212 unit tests plus retired-Spire checks, and smoke test all passed before implementation.
- **Authorized boundary:** only `assets/towerEquations/greekTowers.js` (1,684 lines) moved to strict `.ts` as one source module; its generated sibling, nine focused per-tower tests, roadmap/inventory records, generated `dist/`, and build metadata are included. No file split, formula change, Towers tab, simulation, playfield, main, or balance behavior entered scope, and no shared owner contract edit was required.
- **Dependency audit:** Delta reads gamma definition/equation results with the original two-read `damage` fallback preserved; Epsilon is context-light; Zeta and Eta read their own variables and the gamma result through the presenter; Theta reads its own Aleph variables; Iota reads alpha/beta/gamma connection counts plus its own variables, including its authored keyless Pulse Attack variable, preserved exactly. Registry identity for all six exports is unchanged and remains covered by the existing registry suite.
- **Behavior oracle:** nine deterministic tests were added, increasing the suite from 212 to 221, all passing first against the original JavaScript and then against generated TypeScript output. They characterize each tower independently: exact metadata/variable order including Iota's `undefined` key, every cost curve including Zeta's 10/10/Infinity cascade gate and Eta's 1/5/10 Bet₁ ladder, helper call counts and argument order, native string-coercion through `Math.max` and multiplication, clamps (Zeta speed/total, Eta ring/range, Theta slow/efficacy), exponential/logarithmic/trigonometric formulas computed with identical expressions, sub-equation branches including Eta's ten-line SpdRing block, and golden/base formatting callback order.
- **Contracts:** all six exported definitions end with `satisfies TowerEquationBlueprint`. Non-optionally-chained helper access uses the Phase 24 `requireHelper` guard preserving the original uninitialized-context `TypeError`; raw `unknown` helper reads preserve native `Math.max` coercion; Zeta's critical multiplier preserves native multiplication coercion; Zeta's raw formatter inputs preserve `formatDecimal`'s non-finite fallback via `Reflect.apply`. No `any`, broad assertion, suppression, or duplicate shared contract was added.
- **Output and counts:** Build 758. Counts are 74 TypeScript sources, 74 generated siblings, 160 active authored JavaScript modules, and 45 decision candidates (31.6% of 234 active authored modules converted). The supplemental line snapshot is 11,892 TypeScript lines and 76,859 active JavaScript lines.
- **Automated evidence:** `npm run check:migration-roadmap` passes at 74/74/160/45; `npm run typecheck`, `npm run build`, `npm run lint`, `npm run test:unit` (221/221 plus retired-Spire checks), `npm test`, and `git diff --check` pass. The malformed-storage fixture prints its expected parse diagnostic while the unit command exits 0.
- **Manual evidence:** the recommended all-equation browser overlay pass was not performed; this math-only phase changes no DOM, input, Canvas, audio, save, or Electron path, and every displayed equation string is covered by exact-string compiled-output tests.
- **Residual risk and handoff:** no known behavior gap or temporary compatibility seam remains. The authored equation-definition backlog is now fully typed. Phase 25 is complete; only Phase 26 (pure utilities, tokens, units, and wave schema) is authorized next under its eleven-module card.

### 2026-07-17 — Phase 24 grouped basic/Infinity equation conversion executed

- **Baseline:** clean `claude/js-to-ts-game-conversion-ijybn5` at Phase 23 closeout commit `5ec2f3e` (Build 756). Roadmap check (71/71/163/45), typecheck, build, lint, 203/203 unit tests plus retired-Spire checks, and smoke test all passed before implementation.
- **Authorized boundary:** only `assets/towerEquations/basicTowers.js` and `assets/towerEquations/infinityTower.js` moved to strict `.ts`; their generated siblings, ten focused tests plus a shared grouped-equation test harness, one evidence-backed presenter contract edit, roadmap/inventory records, generated `dist/`, and build metadata are included. No Greek-monolith, Towers tab, simulation, playfield, main, formula, or balance behavior entered scope.
- **Dependency audit:** these modules call context helpers without optional chaining, so each source uses a `requireHelper` guard preserving the original uninitialized-context `TypeError` (characterized by test). Beta's `betSlow` glyph read preserves native `Math.max` coercion over the presenter's `unknown` return; Infinity's `unspentThero` dynamic-context read preserves the original two-read property-getter count (characterized with a stateful getter). Registry identity for `alpha`, `beta`, `gamma`, and `infinity` is unchanged and remains covered by the existing registry suite.
- **Behavior oracle:** ten deterministic tests were added, increasing the suite from 203 to 212, all passing first against the original JavaScript and then against generated TypeScript output. They cover exact metadata/order, every cost curve, glyph-rank helper argument order, connection-count wiring (including Gamma's alpha/beta split), string/`NaN`/Infinity coercion including the slow-field `NaN` passthrough, sub-equation single-object expression+values shape, the fused-glyph lookup order, and base-formatter call order including Beta/Gamma's early `slowText`/`burstText` construction.
- **Contracts:** both sources end each exported definition with `satisfies TowerEquationBlueprint`; the Infinity glyph factory returns the presenter-owned `TowerEquationVariable`. `TowerEquationVariable.renderControlsInline?: boolean` is the only shared owner edit, proven by the compiler and already consumed by the upgrade overlay. No `any`, broad assertion, suppression, or duplicate shared contract was added.
- **Output and counts:** Build 757. Counts are 73 TypeScript sources, 73 generated siblings, 161 active authored JavaScript modules, and 45 decision candidates (31.2% of 234 active authored modules converted). The supplemental line snapshot is 10,169 TypeScript lines and 78,544 active JavaScript lines.
- **Automated evidence:** `npm run check:migration-roadmap` passes at 73/73/161/45; `npm run typecheck`, `npm run build`, `npm run lint`, `npm run test:unit` (212/212 plus retired-Spire checks), `npm test`, and `git diff --check` pass. The malformed-storage fixture prints its expected parse diagnostic while the unit command exits 0.
- **Manual evidence:** optional browser equation-display verification was not performed; this math-only phase changes no DOM, input, Canvas, audio, save, or Electron path.
- **Residual risk and handoff:** no known behavior gap or temporary compatibility seam remains. Phase 24 is complete; only Phase 25 (the Greek grouped equation monolith) is authorized next under its single-file card.
