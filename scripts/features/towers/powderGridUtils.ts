...
/** One falling or resting mote tracked by the Powder simulation. */
export interface PowderGrainRecord {
  id: number;
  x: number;
  y: number;
  size?: number;
...
/** Cached grid metrics captured before a wall change. */
export interface PowderGridMetrics {
  cols?: number;
  rows?: number;
  wallInsetLeftCells?: number;
  wallInsetRightCells?: number;
  scrollOffsetCells?: number;
}

/** Wall measurements published to UI overlays and consumers. */
export interface PowderWallMetrics {
  leftCells: number;
  rightCells: number;
  gapCells: number;
  leftPixels: number;
...
/**
 * Structural surface of the still-JavaScript PowderSimulation consumed by
 * these helpers. Phase 30 will make the class itself the contract owner.
 */
export interface PowderGridSimulation {
  rows: number;
  cols: number;
  grid: number[][];
  wallInsetLeftCells: number;
  wallInsetRightCells: number;
  wallInsetLeftPx?: number;
  wallInsetRightPx?: number;
  cellSize: number;
  width: number;
  height: number;
  scrollOffsetCells: number;
  wallGapCellsTarget?: number | null;
  wallGapReferenceCols?: number;
  grainSizes: number[];
  grains: PowderGrainRecord[];
  computeColliderSize(size: number | undefined): number;
  updateMaxDropSize(): void;
  updateHeightFromGrains(force?: boolean): void;
  render(): void;
  notifyWallMetricsChange(): void;
  [key: string]: unknown;
}
 ...
 */
export function applyWallMask(simulation: PowderGridSimulation): void {
 ...
 * @param simulation - Active PowderSimulation instance.
 */
export function clearGridPreserveWalls(simulation: PowderGridSimulation): void {
  if (!simulation.grid.length) {
    return;
  }
  for (let row = 0; row < simulation.grid.length; row += 1) {
 ...
 */
export function rebuildGridAfterWallChange(
  simulation: PowderGridSimulation,
  previousMetrics: PowderGridMetrics | null = null,
): void {
 ...
 * @param simulation - Active PowderSimulation instance.
 */
export function populateGridFromGrains(simulation: PowderGridSimulation): void {
  if (!simulation.grid.length) {
    return;
  }
  for (const grain of simulation.grains) {
 ...
 */
export function fillCells(simulation: PowderGridSimulation, grain: PowderGrainRecord): void {
 ...
 * @param grain - Grain record with x/y/id/colliderSize fields.
 */
export function clearCells(simulation: PowderGridSimulation, grain: PowderGrainRecord): void {
  if (!grain.inGrid) {
    return;
  }
  const colliderSize = Number.isFinite(grain.colliderSize) ? Math.max(1, Math.round(grain.colliderSize)) : 1;
 ...
 */
export function getSupportDepth(
  simulation: PowderGridSimulation,
  column: number,
  startRow: number,
): number {
 ...
 */
export function getAggregateDepth(
  simulation: PowderGridSimulation,
  startColumn: number,
  startRow: number,
  size: number,
 ...
 */
export function getSlumpDirection(
  simulation: PowderGridSimulation,
  grain: PowderGrainRecord,
): number {
  const colliderSize = Number.isFinite(grain.colliderSize) ? Math.max(1, Math.round(grain.colliderSize)) : 1;
 ...
 */
export function getWallMetrics(simulation: PowderGridSimulation): PowderWallMetrics {
  return {
 ...
 */
export function resolveScaledWallGap(simulation: PowderGridSimulation): number | null {
...
/** Optional flags accepted by the wall-gap appliers. */
export interface ApplyWallGapOptions {
  skipRebuild?: boolean;
}
 ...
 */
export function applyWallGapTarget(
  simulation: PowderGridSimulation,
  options: ApplyWallGapOptions = {},
): boolean {
  if (!simulation.cols) {
    return false;
  }
  ...
  if (isFiniteNumber(gapTarget)) {
    const baseTarget = Math.max(
      largestGrain,
      Math.min(simulation.cols, Math.round(gapTarget)),
    );
 ...
 */
export function setWallGapTarget(
  simulation: PowderGridSimulation,
  gapCells: number,
  options: ApplyWallGapOptions = {},
): boolean {
