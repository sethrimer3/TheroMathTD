 ...
 */
const numberSuffixes: readonly string[] = [
  '',
  'K',
  'M',
  'B',
...
/**
 * Union of the string identifiers accepted as a valid game number notation.
 */
export type GameNumberNotation = (typeof GAME_NUMBER_NOTATIONS)[keyof typeof GAME_NUMBER_NOTATIONS];

/**
 * Listener signature invoked whenever the active notation preference changes.
 */
export type GameNumberNotationListener = (notation: GameNumberNotation) => void;

/**
 * Track available number notations so preference toggles can validate input safely.
 */
const SUPPORTED_GAME_NUMBER_NOTATIONS: ReadonlySet<string> = new Set(Object.values(GAME_NUMBER_NOTATIONS));

/**
 * Tracks the active notation preference for large number formatting.
 */
let currentGameNumberNotation: GameNumberNotation = GAME_NUMBER_NOTATIONS.LETTERS;

/**
 * Holds listener callbacks that react when the notation preference changes.
 */
const notationListeners = new Set<GameNumberNotationListener>();

/**
 * Updates the notation preference and notifies registered listeners.
 * Accepts an unvalidated string (as callers historically passed raw storage/UI
 * values) and falls back to LETTERS when it does not match a known notation.
 * @param notation Requested notation identifier.
 * @returns Resolved notation value after applying the change.
 */
export function setGameNumberNotation(notation: string): GameNumberNotation {
  const normalized: GameNumberNotation = SUPPORTED_GAME_NUMBER_NOTATIONS.has(notation)
    ? (notation as GameNumberNotation)
 ...
 * @returns Active notation identifier.
 */
export function getGameNumberNotation(): GameNumberNotation {
  return currentGameNumberNotation;
}

/**
 * Registers a callback that fires whenever the notation preference changes.
 * @param listener Observer invoked with the new notation string.
 */
export function addGameNumberNotationChangeListener(listener: GameNumberNotationListener): void {
  if (typeof listener === 'function') {
 ...
 * @param listener Observer function to remove from the set.
 */
export function removeGameNumberNotationChangeListener(listener: GameNumberNotationListener): void {
  notationListeners.delete(listener);
}

/**
 * Formats arbitrary values using the suffix table for readability.
 * @param value Raw value sourced from gameplay stats.
 * @returns Nicely formatted number string.
 */
 ...
 * @param tier Magnitude group derived from log10(value) / 3 (1 => thousands).
 * @returns Alphabetic suffix such as "A", "B", ... "AA" for large magnitudes.
 */
function resolveAbcSuffix(tier: number): string {
  let suffix = '';
 ...
 * @param value Raw value provided by the engine.
 * @returns Whole number string safe for UI display.
 */
 ...
 * @param digits Decimal precision to retain.
 ...
 * @param value Ratio expressed as a unit interval.
 * @returns Percentage text without sign enforcement.
 */
 ...
 * @param value Ratio expressed as a unit interval.
 * @returns Percentage text with a + or − sign.
 */
