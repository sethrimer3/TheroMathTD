 ...
 */
export function createTowerEquationTooltipSystem({ tooltipState, getPanelElement = () => null, getUniversalVariableMetadata = () => null, tooltipId = 'tower-upgrade-equation-tooltip', tooltipMarginPx = 12, requestAnimationFrame: requestAnimationFrameOption, } = {}) {
    ...
    const clearScheduledTimeout = typeof window !== 'undefined' && typeof window.clearTimeout === 'function'
    ...
    function buildVariableTooltip(variable, fallbackSymbol = '') {
        ...
        const symbol = (typeof variable.equationSymbol === 'string' && variable.equationSymbol.trim()) ||
            ...
            (typeof variable.key === 'string' && variable.key.trim()
                ...
                : '') ||
            fallback;
        const name = (typeof variable.tooltipName === 'string' && variable.tooltipName.trim()) ||
        ...
        const description = (typeof variable.tooltipDescription === 'string' &&
    ...
    function positionTooltip(target, tooltip) {
        ...
        const clampedLeft = Math.min(Math.max(idealLeft, tooltipMarginPx), Math.max(maxLeft, tooltipMarginPx));
        ...
        const clampedTop = Math.min(Math.max(top, tooltipMarginPx), Math.max(maxTop, tooltipMarginPx));
