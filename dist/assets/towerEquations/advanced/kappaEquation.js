...
export const kappa = {
    ...
    variables: [
        {
            ...
            getSubEquations() {
                const helpers = ctx();
                const gammaRaw = typeof helpers?.calculateTowerEquationResult === 'function'
                    ? helpers.calculateTowerEquationResult('gamma')
                    : 0;
                const betaRaw = typeof helpers?.calculateTowerEquationResult === 'function'
                    ? helpers.calculateTowerEquationResult('beta')
                    : 0;
                const alphaRaw = typeof helpers?.calculateTowerEquationResult === 'function'
                    ? helpers.calculateTowerEquationResult('alpha')
                    : 0;
                const gammaValue = isFiniteNumber(gammaRaw) ? Math.max(0, gammaRaw) : 0;
                const betaValue = isFiniteNumber(betaRaw) ? Math.max(0, betaRaw) : 0;
                const alphaValue = isFiniteNumber(alphaRaw) ? Math.max(0, alphaRaw) : 0;
    ...
    computeResult(values) {
        const gamma = isFiniteNumber(values.gamma) ? values.gamma : 0;
        const beta = isFiniteNumber(values.beta) ? values.beta : 0;
        const alpha = isFiniteNumber(values.alpha) ? values.alpha : 0;
