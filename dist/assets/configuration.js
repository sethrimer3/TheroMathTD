...
import { setTowerDefinitions, setTowerLoadoutLimit, getTowerLoadoutState, getTowerUnlockState, getTowerDefinition, setMergingLogicUnlocked, initializeDiscoveredVariablesFromUnlocks, } from './towersTab.js';
import { setEnemyCodexEntries } from './codex.js';
import { setLevelBlueprints, setLevelConfigs, initializeInteractiveLevelProgression, pruneLevelState, getStartingTheroMultiplier, } from './levels.js';
import { generateLevelAchievements } from './achievementsTab.js';
import { getEmbeddedGameplayConfig, loadGameplayConfigViaFetch, loadGameplayConfigViaModule, } from './gameplayConfigLoaders.js';
...
async function applyGameplayConfigInternal(config = {}) {
    ...
    const defaults = isObjectLike(defaultsSource) ? defaultsSource : {};
    TOWER_LOADOUT_LIMIT =
    ...
    setTowerLoadoutLimit(TOWER_LOADOUT_LIMIT);
    const unlocked = new Set(Array.isArray(defaults.initialUnlockedTowers)
        ? defaults.initialUnlockedTowers.filter((towerId) => Boolean(getTowerDefinition(towerId)))
        : []);
...
export async function ensureGameplayConfigLoaded() {
    ...
    let lastError = null;
    try {
        const configFromFetch = await loadGameplayConfigViaFetch(GAMEPLAY_CONFIG_URL.href, GAMEPLAY_CONFIG_RELATIVE_PATH);
        if (configFromFetch) {
